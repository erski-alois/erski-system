-- ============================================================
-- ERSki 滑雪急診室 預約系統 - 資料庫 Schema v2 (PostgreSQL版)
-- 由 schema.sql(SQLite版)轉換產生，兩份schema要一起維護，不要只改一邊
-- 轉換規則：AUTOINCREMENT->SERIAL；BOOLEAN一律轉INTEGER(維持與SQLite完全相同的0/1語意，
-- 避免程式碼裡大量existing的 col=1 / col=0 比較语句在Postgres底下因型別不符而出錯)；
-- datetime('now')轉成格式相同的UTC字串；3個前向參照的外鍵改用ALTER TABLE補上。
-- ============================================================
...
CREATE TABLE IF NOT EXISTS japan_regions (
    ...
    name TEXT NOT NULL,
    requires_resort_selection INTEGER DEFAULT 1,   -- 是否需要客戶自選雪場
    allow_designate_coach INTEGER DEFAULT 1,       -- 是否開放客戶指定教練(其他雪場=否,只能校方指派)
    requires_accommodation_option INTEGER DEFAULT 0, -- 是否需詢問是否預訂住宿(鬼首=是)
    resort_list_editable INTEGER DEFAULT 0,        -- 雪場清單是否開放後台新增/刪減/修改
...
CREATE TABLE IF NOT EXISTS japan_bookings (
    ...
    equipment_type TEXT CHECK(equipment_type IN ('ski','snowboard')),
    coach_id INTEGER REFERENCES staff(id),
    designate_coach INTEGER DEFAULT 0,
    ...
    rebate_partner_id INTEGER,  -- 退佣對象(合作單位)
...
CREATE TABLE IF NOT EXISTS transactions (
    id SERIAL PRIMARY KEY,
    member_id INTEGER NOT NULL REFERENCES members(id),
    order_id INTEGER,   -- 直接關聯訂單(支援訂金/尾款/退款分次記錄)
    ref_type TEXT,
    ref_id INTEGER,
    amount INTEGER NOT NULL,
...
INSERT INTO pricing_config (config_key, config_value, label) VALUES
 ...
 ('indoor_start_hour', '13', '室內滑雪機台每日開始營業時間(整點)'),
 ('indoor_last_start_hour', '20', '室內滑雪機台每日最後一堂開課時間(整點)'),
 ('indoor_closed_weekdays', '[0]', '室內滑雪機台公休日(0~6對應星期一~星期日,目前為[0]=星期一公休)'),
